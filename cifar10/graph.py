...
def adjacency_2d(dist, idx, locations):
    ...
    for i in range(M):
        for j in range(k):
            t = idx[i][j]
            xdist = (locations[t][0] - locations[i][0])/max_0
            ydist = (locations[t][1] - locations[i][1])/max_1
            L[i][j][0] = (1 - np.abs(xdist));
            L[i][j][1] = (1 - np.abs(ydist));
            L[i][j][2] = (1 - np.abs(xdist)) ** 2 + (1 - np.abs(ydist)) ** 2;
