...
import numpy as np;
import timeit
from models import GCN, ChebyNet, MoNet, VGG, Xception, DSGCDense, DSGCInce, DSGC, DSGCSE
...
parser.add_argument('--mask', action='store_true', default=False, help='append the mask for the CNN model')
...
for iteration in range(max_iter+1):
    ...
    if (iteration % 10 == 0):
        ...
        final_loss = valid_res
            
        print('validation loss %.6f' % (valid_res / (Data.test_num)))

    print("[%3d/%4d] %.2f(m) train_loss %.6f "%
...
print('test: ', final_loss / (Data.test_num));
