...
class Data_utility(object):
    ...
    def __init__(self, args):
        ...
        self.x_train = torch.from_numpy(np.asarray(self.x_train)).float();
        self.y_train = torch.from_numpy(np.asarray(self.y_train)).long();
        self.x_test = torch.from_numpy(np.asarray(self.x_test)).float();
        ...
        self.embed_dim = self.locations.shape[1];
        self.mask = False;
        self.n, self.m = self.x_train.size();
        self.w = 1;
        self.x_train = self.x_train.view((self.train_num, 1, self.m));
        self.x_test = self.x_test.view((self.test_num, 1, self.m));
        self.ch = self.w;
    ...
    def compute_nn_list(self, nn_num):
        ...
        if (self.adjacency == '2d'):
            self.L_idx, self.adj = adjacency_2d(self.dist, self.nn_list, self.locations, self.embed_dim)
            if (self.cuda):
    ...
    def compute_sparse_nn_list(self, args):
        ...
        for i in range(len(self.nn_cfgs)):
            ...
            L_idx, adj = adjacency_2d(dist, nn_list, locations, self.embed_dim)
    ...
    def get_batches(self, data, batch_size, shuffle=True):
        ...
        while (start_idx < length):
            ...
            if (self.adjacency == 'Laplacian' or self.adjacency == 'randomwalk' or self.adjacency == 'sigma'):
